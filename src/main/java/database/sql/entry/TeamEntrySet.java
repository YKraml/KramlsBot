...
public class TeamEntrySet extends AbstractEntrySet<TeamEntrySet.TeamEntry> {
  ...
  public void addSingleResult(ResultSet resultSet) throws SQLException {
    ...
    TeamEntry teamEntry = new TeamEntry(id, name, playerId, teamsize, isInDungeon, money, stardust,
        cookies, dungeon, level,
        morphStones);
  ...
  public static class TeamEntry extends AbstractEntry {
    ...
    public TeamEntry(String id, String name, String playerId, int teamsize, boolean isInDungeon,
        int money, int stardust, int cookies, String dungeon, int level,
        int morphStones) {
      this.id = id;
      this.name = name;
