...
public final class MusicPlayerManagerImpl implements MusicPlayerManager {
  ...
  public void playThisSongNext(ServerVoiceChannel voiceChannel, TextChannel textChannel,
      QueueElement queueElement) {
    MusicPlayer musicPlayer = getPlayerByServer(voiceChannel.getServer()).orElseGet(
        () -> createPlayer(voiceChannel.getServer(), voiceChannel, textChannel));
    musicPlayer.setServerVoiceChannel(voiceChannel);
    musicPlayer.setTextChannel(textChannel);
  ...
  public Queue getQueueByServer(Server server) throws MyOwnException {
    return getPlayerByServer(server).orElseThrow(
        () -> new MyOwnException(new QueueNonExisting(server), null)).getQueue();
  }
  ...
  public void addQueueMessage(Message message, Observer observer) {
    message.getServer().flatMap(this::getPlayerByServer)
        .ifPresent(player -> player.addObserver(observer));
  }
