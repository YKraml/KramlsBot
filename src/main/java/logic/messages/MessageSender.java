...
public interface MessageSender {
  ...
  void sendGroupList(TextChannel channel, Player player, PlayerLoader playerLoader,
      WaifuLoader waifuLoader, JikanFetcher jikanFetcher, MessageSender messageSender,
      Terminal terminal) throws MyOwnException;

  void sendGuessGameStarted(TextChannel textChannel, RoutineRunner routineRunner,
  ...
  void sendWaifuStats(TextChannel channel, Waifu waifu, Player player, PlayerLoader playerLoader,
      WaifuLoader waifuLoader, JikanFetcher jikanFetcher, MessageSender messageSender,
      Terminal terminal) throws MyOwnException;

  void sendTeamKilled(TextChannel textChannel, Team team, int level, Inventory inventory)
      throws MyOwnException;

  void sendTeamIsLow(TextChannel textChannel, Team team) throws MyOwnException;

  void sendDailyAlreadyUsed(TextChannel channel, Player player, String newDate)
      throws MyOwnException;

  void sendDailyUsed(TextChannel channel, Player player) throws MyOwnException;

  Message sendFightRequest(TextChannel textChannel, User user, User userEnemy, long money,
      long stardust, long morphStones) throws MyOwnException;

  void sendGaveWaifu(TextChannel channel, Player senderPlayer, Player receiverPlayer, Waifu waifu)
      throws MyOwnException;
  ...
  void sendWaifusDeleted(TextChannel channel, Player player, int deletedWaifus, int stardust,
      int cookies) throws MyOwnException;

  void sendTeamRenamedMessage(TextChannel channel, Player player, String oldName, String newName)
      throws MyOwnException;
  ...
  void sendGaveMoney(TextChannel channel, Player giverPlayer, Player receiverPlayer, int money)
      throws MyOwnException;

  void sendSongAdded(TextChannel channel, QueueElement queueElement,
  ...
  void sendLikedSongs(TextChannel channel, Player player, Server server, User user,
  ...
  void sendWaifuList(TextChannel channel, Player player, MessageSender messageSender,
  ...
  void sendGroupOverview(TextChannel channel, Group group, Player player, PlayerLoader playerLoader,
      WaifuLoader waifuLoader, JikanFetcher jikanFetcher, MessageSender messageSender,
      Terminal terminal) throws MyOwnException;
}
