  ...
  Answer start(RoutineRunner routineRunner) throws MyOwnException {
    ...
    if (team.isPresent()) {
      if (waifuNumber >= team.get().getFighters().size()) {
        ...
        return new Answer(
            "Someone tried to remove a Waifu from his Team, but the Number was to large");
      }

      if (team.get().isInDungeon()) {
        channel.sendMessage(player.getNameTag() + ", das Team \"" + team.get().getName()
            + "\" ist im Moment in einem Dungeon");
        return new Answer(
            "Someone tried to remove a Waifu from his Team, but the team is in a dungeon");
      }

      Fighter fighter = team.get().getFighters().get(waifuNumber);
      team.get().removeWaifu(fighter.getWaifu());
      channel.sendMessage(player.getNameTag() + ", \"" + fighter.getWaifu().getName()
          + "\" wurde aus deinem Team entfernt");
