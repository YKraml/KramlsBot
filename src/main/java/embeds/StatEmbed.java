...
public class StatEmbed extends EmbedBuilder {
    ...
    public StatEmbed(Player player, User user, Server server) {
        ...
        this.addField("Waifus", player.getWaifuList().size() + " / " + player.getMaxWaifus() + " Stueck");
        this.addField("Items", "Konto: " + player.getInventory().getMoney() + " Euro"
                + " | Stardust: " + player.getInventory().getStardust()
                + " | Cookies: " + player.getInventory().getCookies()
        );
